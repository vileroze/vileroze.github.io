...
class Customizer_Repeater extends WP_Customize_Control {
	...
	//==========
	public $customizer_repeater_custom_title_control = false; 
	// public $customizer_repeater_img_pos_control = false;
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		//============
		// if ( ! empty( $args['customizer_repeater_img_pos_control'] ) ) {
		// 	$this->customizer_repeater_img_pos_control = $args['customizer_repeater_img_pos_control'];
		// }
		if ( ! empty( $args['customizer_repeater_custom_title_control'] ) ) {
			$this->customizer_repeater_custom_title_control = $args['customizer_repeater_custom_title_control'];
		}
			...
			foreach($array as $icon){ ?>
                <div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
                    ...
                    <div class="customizer-repeater-box-content-hidden">
						<?php
						$choice = $image_url = $icon_value = $custom_title = $title = $img_pos = $subtitle = $text = $text2  = $link2 = $link = $shortcode = $repeater = $color = $color2 = '';
						if(!empty($icon->id)){
							$id = $icon->id;
						}
						...
						//===================
						// if(!empty($icon->img_pos)){
						// 	$img_pos = $icon->img_pos;
						// }
						if(!empty($icon->custom_title)){
							$custom_title = $icon->custom_title;
						}
						...
						//=============
						// if($this->customizer_repeater_img_pos_control == true){
						// 	$this->img_pos_control($choice);
						// }
						...
						// if($this->customizer_repeater_img_pos_control==true){
						// 	$this->input_control(array(
						// 		'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Image Position','your-textdomain' ), $this->id, 'customizer_repeater_img_pos_control' ),
						// 		'class' => 'customizer-repeater-img-pos-control',
						// 		'type'  => apply_filters('customizer_repeater_input_types_filter', '', $this->id, 'customizer_repeater_img_pos_control' ),
						// 	), $img_pos);
						// }
						if($this->customizer_repeater_custom_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Custom Title','your-textdomain' ), $this->id, 'customizer_repeater_custom_title_control' ),
								'class' => 'customizer-repeater-custom-title-control',
								'type'  => apply_filters('customizer_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_custom_title_control' ),
							), $custom_title);
						}
